...
class PopularProducts extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<HomeCubit, HomeState>(
      ...
      buildWhen: (previous, current) =>
          current is GettingCourses ||
          current is CoursesLoaded ||
          current is CoursesLoadedError,
      builder: (context, state) {
        if (state is GettingCourses) {
          return Center(
          ...
          return Column(
            ...
            children: [
              _buildSectionHeader(context, cubit),
              verticalSpacing(4),
              SizedBox(
                height: 310.h,
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    return _buildModernCourseCard(
                      courseInfo: state.courses[index],
                      title: state.courses[index].title!,
                      category: state.courses[index].categoryName!,
                      price: state.courses[index].price!.toString(),
                      courseHours:
                          state.courses[index].durationInHours!.toString(),
                      instructorName: state.courses[index].instructorName!,
                      rating:
                          double.parse(state.courses[index].rating!.toString()),
                      imageAsset:
                          "https://elcentre-learn.vercel.app/api${state.courses[index].thumbnail!}",
                      context: context,
                    );
                  },
                ),
        ...
        } else if (state is CoursesLoadedError) {
          return _buildErrorState(state.error, context: context);
        }
  ...
  Widget _buildSectionHeader(BuildContext context, HomeCubit cubit) {
    return Padding(
  ...
  Widget _buildErrorState(String error, {required BuildContext context}) {
    return Container(
